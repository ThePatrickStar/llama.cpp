...
static void ggml_cuda_mul_mat_id_mmvq_chunked(
    ...
    if (logical_ids != nullptr) {
        ...
        GGML_ASSERT(logical_ids->ne[0] == ids->ne[0] && logical_ids->ne[1] == ids->ne[1]);
    ...
    for (int64_t first = 0; first < src1->ne[2]; ) {
        ...
        if (service != nullptr) {
            ...
            std::vector<int32_t> logical_host((size_t) logical_ids->ne[0] * (size_t) count);
            std::vector<int32_t> slot_host(logical_host.size());
            for (;;) {
                CUDA_CHECK(cudaMemcpy2DAsync(
                    logical_host.data(), row_bytes,
                    (const char *) logical_ids->data + first * logical_ids->nb[1], logical_ids->nb[1],
                    row_bytes, count, cudaMemcpyDeviceToHost, ctx.stream()));
                CUDA_CHECK(cudaStreamSynchronize(ctx.stream()));
                const int service_rc = service(service_data, logical_host.data(), slot_host.data(),
                                               logical_ids->ne[0] * count);
                if (service_rc > 0) {
                    break;
                }
                if (service_rc < 0 || count == 1) {
                    GGML_ABORT("prefill expert service failed");
                }
                count = std::max<int64_t>(1, count / 2);
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        ggml_mul_mat_id_prefill_service_t service = nullptr;
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_MUL_MAT_ID) {
            if (ggml_mul_mat_id_get_prefill_service(node, nullptr, nullptr)) {
